...
export default function GitPDN() {
        ...
        const fetchGitPDNs = async () => {
            ...
            try {
                ...
                const response = await sendApiRequest(
                    `http://localhost:8080/api/pdn/all/workspace/${prefix}`,
                    null,
                    { method: "GET" }
                );
    ...
    const filteredAndSortedPDNs = useMemo(() => {
        let filtered = gitPDNs.filter(pdn => {
            // Text search
            const matchesSearch = pdn.pdnId.toLowerCase().includes(searchTerm.toLowerCase()) ||
            ...
            // Creator filter
            const matchesCreator = !filters.createdBy || pdn.createdByFirstName === filters.createdBy;

            // Status filter
            const matchesStatus = !filters.status || pdn.currentStatus === filters.status;

            // Date range filter
            const pdnDate = new Date(pdn.createdDate);
            const matchesDateFrom = !filters.dateFrom || pdnDate >= new Date(filters.dateFrom);
            const matchesDateTo = !filters.dateTo || pdnDate <= new Date(filters.dateTo);

            return matchesSearch && matchesCreator && matchesStatus && matchesDateFrom && matchesDateTo;
        });
                ...
                <div className="relative" ref={filterRef}>
                    ...
                    <div className="relative">
                        ...
                        <button
                            ...
                            className={`absolute right-3 top-1/2 transform -translate-y-1/2 p-2 rounded-lg transition-all duration-200 ${hasActiveFilters || showFilters
                            ...
                            >
                                <div className="grid grid-cols-1 lg:grid-cols-6 gap-3 items-center">
                                    ...
                                    <div className="lg:col-span-2">
                                        <p className="text-slate-700 dark:text-slate-300 font-medium text-xs mb-1">Description</p>
                                        <p className="text-slate-600 dark:text-slate-400 text-sm line-clamp-1" title={pdn.description}>
                                            {pdn.description}
                                        </p>
                                    </div>
                                    ...
                                    {/* Status */}
                                    <div className="lg:col-span-1">
                                        <p className="text-slate-700 dark:text-slate-300 font-medium text-xs mb-1">Status</p>
                                        <span className={`px-2 py-1 rounded-lg text-xs font-medium backdrop-blur-sm border ${getStatusVariant(pdn.currentStatus)}`}>
                                            {pdn.currentStatus}
                                        </span>
                                    </div>
                                    ...
                                    <div className="lg:col-span-1">
                                        <div className="space-y-1">
                                            <div>
                                                <p className="text-slate-700 dark:text-slate-300 font-medium text-xs">Created by</p>
                                                <p className="text-slate-600 dark:text-slate-400 text-xs">{pdn.createdByFirstName}</p>
                                            </div>
                                            <div>
                                                <p className="text-slate-700 dark:text-slate-300 font-medium text-xs">Assigned to</p>
                                                <p className="text-slate-600 dark:text-slate-400 text-xs">{pdn.currentOwnerFirstName}</p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    {/* Date */}
                                    <div className="lg:col-span-1">
                                        <p className="text-slate-700 dark:text-slate-300 font-medium text-xs mb-1">Created</p>
                                        <p className="text-slate-500 dark:text-slate-500 text-xs">{pdn.createdDate}</p>
                    ...
                    {filteredAndSortedPDNs.length === 0 && (
                        <div className="text-center py-12">
                            <div className="bg-white/50 dark:bg-slate-800/50 backdrop-blur-sm rounded-xl p-8 max-w-sm mx-auto">
                                ...
                                <p className="text-slate-600 dark:text-slate-400 mb-3">No PDNs found</p>
                                <Button asChild className="bg-gradient-to-r from-blue-500 to-purple-500 hover:from-blue-600 hover:to-purple-600 border-0 rounded-lg text-sm">
